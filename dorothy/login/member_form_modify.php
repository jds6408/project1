    ...
    <script>
        $(document).ready(function() {
            ...
            $("#id").keyup(function() { // id입력 상자에 id값 입력시
                ...
                $.ajax({ //실시간으로 db에 왓다갔다 가능
                    type: "POST",
                    url: "check_id.php", //post방식으로 url로 보내는거 
                    data: "id=" + id,
                    cache: false,
                    success: function(data) // 위에 것이 잘넘어갔으면 
                    {
                        //data => echo "문자열" 이 저장된
                        $("#loadtext").html(data);
                    }
...
<?
    ...
    $hp = explode("-", $row[hp]);  //000-0000-0000 문자열을 기준으로 나눠주는 배열 함수
    $hp1 = $hp[0]; //000
    $hp2 = $hp[1]; //0000
    $hp3 = $hp[2]; //0000

    $email = explode("@", $row[email]);
    $email1 = $email[0]; //jds6408
